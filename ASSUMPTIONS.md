...
|---|---|---|---|
| 🟢 | **Real data** | measured & published, used as-is | storm pipe network (the 7 real-network cities); ground elevation; rainfall & temperature; parcel & building footprints; node / outfall / catch-basin locations |
...
|---|---|---|
| **Storm network** (pipes, nodes, outfalls) | 🟢 Real (7 cities) · 🟠 synthesized elsewhere | Real = published inverts, diameters, materials, locations. Honest gap-fills: ~7–10% of missing inverts are slope/neighbour-interpolated; dangling node refs snap to pipe geometry; non-circular profiles → equivalent circular (original shape kept in diagnostics). |
...
All 7 real-network cities use **real pipes** (🟢). They differ only in how subcatchments and
...
> [!NOTE]
> A generated model is **grounded in real data and ready to run**: the pipes (in the 7 cities),
...
> partitioning** and, outside the 7 cities, the **network** itself.
