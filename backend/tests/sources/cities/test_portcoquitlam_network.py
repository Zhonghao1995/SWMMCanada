"""Tests for the Port Coquitlam -> SWMM NetworkIn adapter, on real downtown fixtures
(615 storm mains, recorded 2026-07-28)."""
import json
...
from swmmcanada.sources.cities.portcoquitlam import (
    _elev,
    build_portcoquitlam_network,
    fetch_portcoquitlam_land,
    fetch_portcoquitlam_sanitary,
    fetch_portcoquitlam_storm,
)

FIXTURES = Path(__file__).resolve().parents[2] / "fixtures" / "portcoquitlam"
BBOX = (-122.785, 49.255, -122.765, 49.270)
...
def result():
    return build_portcoquitlam_network({"mains": _load("mains"), "manholes": _load("manholes")})
...
def test_minus99_sentinel():
    assert _elev(-99) is None
    assert _elev(6.82) == pytest.approx(6.82)


def test_real_inverts_and_rims(result):
    inv = [j.invert_m for j in result.network.junctions]
    assert 0 < min(inv) < max(inv) < 60
...
def test_sanitary_fixture_builds():
    res = build_portcoquitlam_network({"mains": _load("sanitary_mains")})
...
def test_fetch_routing():
    client = FakeClient()
    out = fetch_portcoquitlam_storm(BBOX, client=client)
    urls = [u for u, _ in client.calls]
    assert any("Drainage_Network/FeatureServer/0" in u for u in urls)
    assert any("Drainage_Network/FeatureServer/5" in u for u in urls)
    assert set(out) == {"mains", "manholes"}

    client2 = FakeClient()
    fetch_portcoquitlam_sanitary(BBOX, client=client2)
    assert any("Sanitary_Network2/FeatureServer/0" in u for u, _ in client2.calls)

    client3 = FakeClient()
    land = fetch_portcoquitlam_land(BBOX, client=client3)
    assert any("Drainage_Network/FeatureServer/6" in u for u, _ in client3.calls)
    assert any("Cadastral/FeatureServer/0" in u for u, _ in client3.calls)
