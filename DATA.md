...
| Storm + buildings | City of Kitchener Open Data | real storm network, buildings | OGL – Kitchener |
| Stormwater + Property Fabric/Buildings | City of Waterloo Open Data | real storm network, parcels, buildings | Waterloo Open Data User Licence |
...
| **Kitchener, ON** | `services1.arcgis.com/qAo1OsXi67t7XgmS/.../FeatureServer` (the City of Kitchener's own org — **not** a regional feed) | `Storm_Pipes` (UP/DN_INVERT) · `Storm_Manholes` · `Storm_Outlets` · `Storm_Catchbasins` · `Sanitary_Pipes`, ACTIVE GRAVITY (separated **sanitary** skeleton, second tagged system) | explicit integer node IDs | `Building_Outlines` only (no parcel polygons) | OGL – Kitchener |
| **Waterloo, ON** | `services.arcgis.com/ZpeBVw5o1kjit7LT` ([data.waterloo.ca](https://data.waterloo.ca)) | `Stormwater_Gravity_Mains`, `ASSET_TYPE = 'Collector'` (UPSTREAM/DOWNSTREAMINVERT — **two missing sentinels, 0 and 111.111**, screened by a 290–420 m band; HEIGHT/WIDTH mm; CROSSSECTIONSHAPE; catch-basin leads, culverts, ditches and site drains stay out) · `Stormwater_Manholes` (TOPOFMH rim → depths; ENTNAME structure ids → node labels) · `Stormwater_Outlets` · `Stormwater_Catch_Basins` seeds · `Sanitary_Sewer_Gravity_Mains` (separated **sanitary** skeleton, second tagged system) | geometry-inferred, labelled ends | `Property_Fabric/1358` lots (ROAD/RAILWAY parcels left out) · `Buildings` | Waterloo Open Data User Licence |
...
| **Strathcona County, AB** | `services.arcgis.com/B7ZrK1Hv4P1dsm9R` (hub with licence page) | `Storm_Gravity_Main`, pipetype Collector/Transmission/Conduit/Culvert (upinvert/downinvert — the wave's patchiest; 0 = missing) · `Storm_Manhole` (rimelev) · `Storm_Discharge_Point` outfalls (**inlet-side rows dropped by the downhill-end filter**) · `Storm_Catch_Basin` seeds · `Waste_Water_Gravity_Main` (separated **sanitary** skeleton, second tagged system) | geometry-inferred | `Building_Footprints` (no parcels) | Open Data – Strathcona County |
| **Windsor, ON** | [opendata.citywindsor.ca](https://opendata.citywindsor.ca) — static `/Uploads/Sewer Pipeline.zip` dump (**download-and-cache**, no query API) | EPSG:26917 shapefile: Upstream_E/Downstre_1 inverts in a (150,220) m band (0 sentinel; deep combined interceptors dip under the Detroit River) · Upstream_M/Downstream id labels · `Sewer_Type` STORM+COMBINED join the storm graph (ADR 0021), SANITARY = tracer, ABANDONED/PRIVATE out | geometry topology, labelled ends | none consumed (parcel/building zips exist — future enrichment) | Open Data – Windsor |
| **District of North Vancouver, BC** | [geoweb.dnv.org/data](https://geoweb.dnv.org/data/) — static `/Products/Data/SHP/*.zip` dumps (**download-and-cache**, actively refreshed) | `StmMain_shp.zip` (UP/DN_ELEV string inverts, **sentinel -99**, 44%; AM_SIZE string mm; spelled-out AM_MATERIA) · `SanMain_shp.zip` (separated **sanitary** skeleton, second tagged system) · no rims/seeds consumed (fitting dump ships bare type codes) — junction delineation | geometry-inferred | CadParcels/BldBuilding zips exist (future enrichment) | Open Data – DNV |
...
Kitchener and Waterloo each publish their own network on their own org; the Kitchener feed's
`OWNERSHIP = WATERLOO` rows are 38 border pipes, not Waterloo's network (Cambridge publishes
none yet and synthesizes). The two cities share a diagonal border no pair of boxes can split,
so Waterloo dispatches on its municipal boundary polygon. How each
