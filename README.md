...
|---|---|---|
| **Real network** | uses the city's published storm pipes (real inverts, diameters, manholes, and outfalls) | **20 cities** that publish a storm network: Victoria, Ottawa, Toronto, Calgary, Surrey, London, Kitchener–Waterloo, Kelowna, Regina, Vancouver, Coquitlam, Burnaby, New Westminster, Nanaimo, Barrie, Abbotsford, Saskatoon, Kingston, Peterborough, and Reykjavík (IS) — the first international city, on Iceland's national *fitjuskrá* schema |
...
backend/swmmcanada/      # Python pipeline: open data -> SWMM model
  ...
  sources/     live data adapters (climate, DEM incl. 1 m LiDAR auto-select, ECCC IDF design storms, land cover, soil, OSM streets)
    cities/    base.py (shared assembler) + 20 real-network adapters (victoria · ottawa · toronto · calgary · surrey · london · kitchener · kelowna · regina · vancouver · coquitlam · burnaby · newwestminster · nanaimo · barrie · abbotsford · saskatoon · kingston · peterborough · reykjavik)
  ...
  pipeline.py  build_from_aoi · build_from_<city> (20 real-network cities)
